...
void unit_action_match(int com_id, const std::string &json_str)
{
    ...
    // ------- 这里开始是修改后的 simdjson 取字符串逻辑 -------

    std::string request_id;
    {
        auto result = doc["request_id"].get_string();  // simdjson_result<std::string_view>
        error = result.error();
        if (error) {
        ...
        request_id = std::string(result.value());
    }

    std::string work_id;
    {
        auto result = doc["work_id"].get_string();
        error = result.error();
        if (error) {
        ...
        work_id = std::string(result.value());
    }
    if (work_id.empty()) work_id = "sys";

    std::string action;
    {
        auto result = doc["action"].get_string();
        error = result.error();
        if (error) {
        ...
        action = std::string(result.value());
    }
